...
extern "C" fn rust_sock_notify_fn(
    ...
    errorno: i32)
{
    match HANDLE_HASH.lock().unwrap().get(&(private_data as u64)) {
	Some(h) =>  {
	    let res = if error == 0 {
		Ok(())
	    } else {
	    ...
	    // Call user fn
	    match h.sock_notify_fn {
		Some(f) => f(h.sock_notify_private_data,
			     datafd,
			     channel,
			     TxRx::new(tx_rx),
			     res),
	...
	Some(h) =>  {
		...
		Some(f) => {
			...
			*dst_host_ids_entries = hosts_vec.len();
			...
			for i in 0..hosts_vec.len() {
			    *retvec = hosts_vec[i].host_id;
			    retvec = retvec.offset(size_of::<u16>() as isize);
			}
		    }
		},
		None => {}
    ...
    data_mtu: u32)
{
	...
	Some(h) =>  {
	    // Call user fn
	    match h.pmtud_notify_fn {
		Some(f) => f(h.pmtud_notify_private_data,
			     data_mtu),
		None => {}
	    }
	},
    ...
    onwire_ver: u8)
{
	...
	Some(h) =>  {
	    // Call user fn
	    match h.onwire_notify_fn {
		Some(f) => f(h.onwire_notify_private_data,
			     onwire_min_ver,
			     onwire_max_ver,
			     onwire_ver),
		None => {}
    ...
    external: u8)
{
	...
	Some(h) =>  {
	    // Call user fn
	    match h.host_status_change_notify_fn {
		Some(f) => f(h.host_status_change_notify_private_data,
			     HostId{host_id},
    ...
    external: u8)
{
	...
	Some(h) =>  {
	    // Call user fn
	    match h.link_status_change_notify_fn {
		Some(f) => f(h.link_status_change_notify_private_data,
			     HostId{host_id},
			     link_id,
		  ...
		  flags: HandleFlags) -> Result<Handle>
{
	...
	let rhandle = PrivHandle{log_fd: log_fd,
				 sock_notify_fn: None,
...
pub fn get_crypto_list() -> Result<Vec<CryptoInfo>>
{
	...
	for i in 0..list_entries {
	    retvec.push(CryptoInfo::new(c_list[i]));
	}
	Ok(retvec)
    } else {
	Err(Error::last_os_error())
...
pub fn get_compress_list() -> Result<Vec<CompressInfo>>
{
	...
	for i in 0..list_entries {
	    retvec.push(CompressInfo::new(c_list[i]));
	}
	Ok(retvec)
    } else {
	Err(Error::last_os_error())
...
pub fn host_get_host_list(handle: Handle) -> Result<Vec<HostId>>
{
	...
	for i in 0..c_host_ids_entries {
	    host_vec.push(HostId {host_id: c_host_ids[i]});
	}
	Ok(host_vec)
    } else {
	Err(Error::last_os_error())
...
pub fn get_transport_list() -> Result<Vec<TransportInfo>>
{
	...
	for i in 0..list_entries {
	    retvec.push(TransportInfo::new(c_list[i]));
	}
	Ok(retvec)
    } else {
	Err(Error::last_os_error())
...
/// Enable/disable a link (you still need to call [handle_set_fwd] for traffic to flow
...
// time_t varies vetween platforms so we convert here
fn to_timet(t: i64) -> ffi::time_t
{
    match t.try_into() {
	Ok(v) => return v,
	Err(_e) => return 0 as ffi::time_t,
    }
}
			    ...
			    interval: i64, timeout: i64, precision: u32) -> Result<()>
{
	...
	ffi::knet_link_set_ping_timers(handle.knet_handle as ffi::knet_handle_t,
				       host_id.host_id, link_id,
				       to_timet(interval), to_timet(timeout), precision)
    };
    if res == 0 {
	Ok(())
    } else {
...
pub fn link_get_link_list(handle: Handle, host_id: &HostId) -> Result<Vec<u8>>
{
	...
	for i in 0..c_link_ids_entries {
	    link_vec.push(c_link_ids[i]);
	}
	Ok(link_vec)
    } else {
	Err(Error::last_os_error())
